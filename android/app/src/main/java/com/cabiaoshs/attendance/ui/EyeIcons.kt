...
val VisibilityIcon: ImageVector by lazy { buildEye(off = false) }

/** Eye-with-slash icon (password hidden). */
val VisibilityOffIcon: ImageVector by lazy { buildEye(off = true) }

private fun buildEye(off: Boolean): ImageVector =
    ImageVector.Builder(
        name = if (off) "VisibilityOff" else "Visibility",
        defaultWidth = 24.dp,
        defaultHeight = 24.dp,
        viewportWidth = 24f,
        viewportHeight = 24f,
    ).addPath(
        pathData = PathParser().parsePathString(if (off) VISIBILITY_OFF else VISIBILITY_ON).toNodes(),
        fill = SolidColor(Color.Black),
    ).build()

private const val VISIBILITY_ON =
